...
>(({ className, ...props }, ref) => {
  return (
    <main
      ref={ref}
      className={cn(
        "relative flex h-full w-full flex-1 flex-col bg-background",
  ...
  (
    ...
    const button = (
      <Comp
        ...
        className={cn(
          sidebarMenuButtonVariants({ variant, size: effectiveSize }),
          className,
        )}
        {...props}
      />
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        // "group-data-[collapsible=icon]:hidden",
        showOnHover &&
          "opacity-0 group-hover/menu-item:opacity-100 data-[state=open]:opacity-100 peer-data-[active=true]/menu-button:text-sidebar-accent-foreground",
        className,
      )}
      {...props}
    />
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  // Use a fixed width initially to avoid hydration mismatch, then randomize on client
